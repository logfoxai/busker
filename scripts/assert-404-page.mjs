#!/usr/bin/env node
/**
 * Built Starlight 404 must be a dedicated page — not the homepage demo pile.
...
const forbidden = ['splash__demo', 'data-mock', 'busk(root,'];
for (const needle of forbidden) {
	if (html.includes(needle)) {
		console.error(`assert-404-page: 404.html must not include homepage demo marker: ${needle}`);
		process.exit(1);
	}
}
