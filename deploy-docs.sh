...
#
# Order matters. Astro emits content-hashed assets under `_astro/`. A naive
# `aws s3 sync --delete` can remove an old hash while CloudFront is still
# serving HTML that points at it → CSS 404 → fully unstyled page. Publish new
# hashes first, swap HTML second, invalidate third, and never delete `_astro/`
# objects in the same deploy.
